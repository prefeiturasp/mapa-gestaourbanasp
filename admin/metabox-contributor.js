jQuery(document).ready(function() {
    ...
    // Hide editor buttons
    $('#ed_toolbar, #post-status-info').hide();

    //Tags - Adição de texto ao .howto (Help)

    $('.jaxtag .howto').append('. Exemplos: Mobilidade Urbana, Cultura, Habitação, Esporte, Meio Ambiente.').val();

    // SB Uploader
    $('#sb_uploader_meta_box_1').append('<p class="howto" id="uploader-howto">O envio de imagem não é obrigatório. O limite máximo de tamanho do arquivo é de <strong>2MB</strong></p>')

    // Checa as opções defaults para Soluções e Problemas

        var chkbox = QueryString("mycat");

        if(chkbox=="1"){

            //Category
            document.getElementById("in-category-50").checked=false;
            document.getElementById("in-category-1").checked="checked";

            //Pins
            document.getElementById("pin_2007").checked=false;
            document.getElementById("pin_2009").checked="checked";
        }
        else
        {   //Category
            document.getElementById("in-category-1").checked=false;
            document.getElementById("in-category-50").checked="checked";

            //Pins
            document.getElementById("pin_2009").checked=false;
            document.getElementById("pin_2007").checked="checked";
        }
