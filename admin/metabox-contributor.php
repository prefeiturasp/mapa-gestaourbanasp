...
if ( $user->roles[0] == 'contributor' ) {
	add_filter ( 'user_can_richedit' , create_function ( '$a' , 'return false;' ) , 10 );
	...
	add_action( 'add_meta_boxes', 'change_submit_meta_box', 0 );
	function change_submit_meta_box() {
		global $wp_meta_boxes;
		unset( $wp_meta_boxes['post']['side']['core']['submitdiv'] );
		add_meta_box('submitdiv',		__('Enviar contribuição'),
	...
	function custom_admin_post_thumbnail_html( $content ) {
		$content = '<br />' . str_replace( __( 'Set featured image' ), __( 'Adicionar imagem' ), $content);
		return $content;
	...
	function my_post_admin_notices() {
		...
		switch (absint($_GET['my_message'])) {
			case 1:
				$message = 'Sua contribuição pode demorar algumas horas para ir para o mapeamento. Enquanto isso, feche esta janela e navegue pelas outras contribuições...';
				break;
			default:
				$message = 'Unexpected error';
		}
		echo '<div id="notice" class="updated"><p>' . $message . '</p></div>';
	}
}
...
// Mapa de Vista Metabox Custom
function mapasdevista_add_custom_box() {

	//Add default metabox
	//mapasdevista_metabox_default();
	...
	foreach ($post_types as $post_type)
		add_meta_box( 'mapasdevista_metabox', __( 'Indique o local da sua contribuição', 'mapasdevista' ), 'mapasdevista_metabox_map', $post_type, 'normal', 'core' );
...
// Mapa de Vista Metabox Default

function mapasdevista_metabox_default() {
	?>
	<div id="titlediv">
		<div id="titlewrap">
			<label class="" id="title-prompt-text" for="title"
				style="text-shadow: none;">Título</label> <input type="text"
				name="post_title" size="30" value="" id="title" autocomplete="off">
		</div>
	</div>

	<div id="postdivrich" class="postarea">
		<div id="wp-content-wrap" class="p-core-ui wp-editor-wrap html-active">
			<textarea class="wp-editor-area" cols="40" name="content" id="content"></textarea>
		</div>
	</div>
<?php
}
...
function mapasdevista_metabox_map() {
	...
	<!-- Latitude e Longitude	(ocultos, mas precisam de validação) -->
	<fieldset style="display:none">
		<label for="mpv_lat"><?php _e('Latitude', 'mpv');?>:</label> <input
			type="text" class="medium-field" name="mpv_lat" id="mpv_lat"
			value="<?php echo $location['lat'];?>" /> <label for="mpv_lon"><?php _e('Longitude', 'mpv');?>:</label>
		<input type="text" class="medium-field" name="mpv_lon" id="mpv_lon"
			value="<?php echo $location['lon'];?>" /> <input type="button"
			id="mpv_load_coords" value="Exibir" />
	</fieldset> 
	
	<!-- Endereço (serve apenas para definir longitude e latitude) -->
	<fieldset>
		<label for="mpv_search_address"><?php _e('Digite o endereço', 'mpv');?>:</label>
		<input type="text" id="mpv_search_address" class="large-field" />
		<p class="howto">Aperte a tecla "enter" para confirmar a localização no mapa. Você pode também clicar no mapa e escolher a localização manualmente. Importante: Não esqueça de conferir se o marcador aparece no mapa antes de enviar sua contribuição.</p>
	</fieldset>
...
<?php if (count($pins) >  0) : ?>
<h4>
	<?php _e("Available pins", "mapasdevista");?>
</h4>
...
<div class="iconlist">
	...
	<div class="icon">
		...
		<div class="icon-image">
			<label for="pin_<?php echo $pin->ID;?>"> <?php echo wp_get_attachment_image($pin->ID, 'full', false, array('style'=>'max-width:64px;max-height:64px;'));?>
			</label>
		</div>
		<div class="icon-info">
			<input type="radio" name="mpv_pin" id="pin_<?php echo $pin->ID;?>"
				value="<?php echo $pin->ID;?>"
				<?php if($post_pin==$pin->ID) echo ' checked';?> /> <span
				class="icon-name"><?php echo $pin->post_name;?> </span>
		</div>
	</div>
	<?php endforeach;?>
</div>
...
<div style="display: none">
	<h4>
		<?php _e("Maps", "mapasdevista");?>
	</h4>
	<p>
		<?php _e('Mark the maps in which this post is going to appear', 'mapasdevista'); ?>
	</p>
	<ul>
		...
		<li><input type="checkbox" name="mpv_inmap[]"
			value="<?php echo $map['page_id']; ?>"
			id="inmap_<?php echo $map['page_id']; ?>"
			<?php if (in_array($map['page_id'], $inmaps) || count($maps == 1)) echo 'checked'; ?> />
			<label for="inmap_<?php echo $map['page_id']; ?>"> <?php echo $map['name']; ?>
		</label>
		</li>
...
<div class="iconlist" id="image-maps">
	...
	<div class="icon">
		<div class="icon-image">
			...
			<input type="hidden" class="full_image_src"
				value="<?php echo $image_full_src;?>" />
		</div>
		<div class="icon-info">
			<input type="checkbox"
				name="mpv_img_coord[<?php echo $map['page_id']?>]"
			...
			<label for="mpv_img_coord_<?php echo $map['page_id']?>"><?php echo $map['name']?>
			</label> <input type="hidden"
				name="mpv_img_pin[<?php echo $map['page_id']?>]"
...
<div id="dialog">
	<div class="iconlist">
		...
		<div class="icon" id="icon-<?php echo $pin->ID;?>">
			<?php echo wp_get_attachment_image($pin->ID, array(64,64));?>
		</div>
		<?php endforeach;?>
	</div>
	<div class="panel">
